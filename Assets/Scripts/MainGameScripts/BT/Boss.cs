...
public class Boss : BTRunner
{
    ...
    .Selector("Root")
        .Guard(IsAlive, root =>
        {
            root.Guard(HasTarget, b =>
            {
                b.Sequence("AttackCombo")   //AttackCombo => 페이즈 1
...
/*
 코드 분석
-   behaㅍior 트리 전체적인 틀 같은 느낌
-   BtRunner 클래스를 상속함
-   Player가 보스의 인식 거리 내에 들어올 시 공격
-   공격 패턴 노드 > 
            75줄 Wide SafeZone = 장판/안전지대
            96줄 DropSolo = 플레이어 따라 낙하 공격
            127줄 SideSweep L→R = 좌>우 낙하??
            146줄 SideSweep R→L = 우>좌 낙하>?
            164줄 DropMultiple = 맵 전체 낙하 공격
 
 
 추가
-   실제로 작동 하게 만들기
-   보스의 체력과 그에 따른 페이즈 분기 나누기

-   낙하공격이 겹침 > dropsolo 노드 수정 >> 낙하공격을 플레이어를 추적하는 미사일형태로 수정
-   sidesweep 페이즈 2 진입시 속도 증가

-   페이즈 2 진입시 카메라 연출 추가(되면)
